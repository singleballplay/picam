...
def adjust_video_setting(device, setting, value):
    ...
    """
    result = subprocess.run(['v4l2-ctl', '-d', device, '-c', '{}={}'.format(setting, value)])
...
def get_v4l2_settings(device):
    ...
    for line in v4l2_options_raw:
        ...
        if v4l2_option and v4l2_option != 'User' and v4l2_option != 'Camera':
            _min = re.findall(r'min=(-?\d+)', line)
            _max = re.findall(r'max=(-?\d+)', line)
            _default = re.findall(r'default=(-?\d+)', line)
            _step = re.findall(r'step=(-?\d+)', line)
            _value = re.findall(r'value=(-?\d+)', line)
...
def v4l2h264enc_available():
    cmd = subprocess.run('gst-inspect-1.0 v4l2h264enc'.split(' '), stdout=subprocess.PIPE)
...
class VideoDeviceHandler(MethodView):
    ...
    def get(self, serial):  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        v4l2_options = {}
        video_options = None
        for _, device_info in find_video_devices().items():
